...
class TestMenu(unittest.TestCase):
    def test_menu_display(self):
        """
        Testa a função Menu.display simulando entradas do usuário.
        """
        simulated_input = "1\n2\n3\n0\n"  # Simula as opções 1, 2, 3 e 0
        sys.stdin = StringIO(simulated_input)  # Redireciona stdin para simular entrada

        try:
            Menu.display()  # Chama a função para testar
        except Exception as e:
            self.fail(f"Menu.display falhou com exceção: {e}")
        finally:
            sys.stdin = sys.__stdin__  # Restaura stdin original
